...
/**
 * GIA V4.9 - CLEAN LAYOUT & NAV FIX
 * Feature: Better Grid System, Centered Text, Robust Navigation
 */
...
if (!savedState || !savedState.version || savedState.version < 4.9) {
    const baseState = savedState || DEFAULT_DB;
    // Migration logic basic
    state = { ...DEFAULT_DB, users: baseState.users || DEFAULT_DB.users, modules: baseState.modules || DEFAULT_DB.modules };
...
function getStudentAnalytics(user) {
    ...
    let message = `Analyse en cours (${validatedCount}/${MIN_VALIDATIONS})`;
...
function renderDashboard() {
  ...
  return `
    <div class="h-full flex flex-col p-8 bg-slate-50 fade-in overflow-hidden">
        <header class="flex-shrink-0 mb-8 flex items-center justify-between">
            <div><h1 class="text-4xl font-black text-slate-900 mb-1">Tableau de Bord</h1><p class="text-base text-slate-500 font-bold">Vue d'ensemble de votre parcours.</p></div>
        ...
        <div class="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 flex flex-col gap-8 h-full">
                ...
                <!-- CARTE DE REPRISE (HAUT) -->
                <div class="flex-1 bg-white p-10 rounded-[2.5rem] shadow-premium border border-slate-100 relative overflow-hidden group cursor-pointer flex flex-col justify-center items-start transition-transform hover:scale-[1.01]" onclick="setView('classroom')">
                    <div class="relative z-10 max-w-2xl">
                        ...
                        <h2 class="text-5xl font-black text-slate-900 mb-6 leading-tight">${currentLesson.title}</h2>
                        <p class="text-slate-500 text-xl mb-10 font-medium leading-relaxed max-w-lg">${currentLesson.subtitle}</p>
                        <button class="bg-slate-900 text-white px-10 py-5 rounded-2xl font-black flex items-center gap-4 hover:gap-6 transition-all shadow-xl hover:shadow-2xl text-lg">
                            Accéder au cours <i data-lucide="arrow-right"></i>
                        </button>
                    </div>
                ...
                <!-- SMART FORECAST (BAS) -->
                <div class="${analytics.colorClass} p-8 rounded-[2rem] border shadow-sm flex flex-col justify-center relative overflow-hidden transition-colors duration-500 min-h-[180px]">
                    ${analytics.isCalibrating ? `
                        <div class="relative z-10 w-full max-w-3xl mx-auto text-center">
                            <div class="flex flex-col items-center gap-4 mb-6">
                                <div class="w-14 h-14 rounded-full bg-white/80 backdrop-blur shadow-sm flex items-center justify-center animate-spin"><i data-lucide="loader-2" class="w-7 h-7 text-slate-400"></i></div>
                                <div>
                                    <h3 class="text-2xl font-black text-slate-800">Calibrage de l'IA</h3>
                                    <p class="text-slate-400 font-bold">Nous analysons votre rythme de travail...</p>
                                </div>
                            </div>
                            <div class="w-full bg-slate-200/50 h-5 rounded-full overflow-hidden border border-slate-200">
                            ...
                            <p class="text-sm text-slate-400 mt-3 font-bold uppercase tracking-wider">Encore ${analytics.minValidations - analytics.validatedCount} validations nécessaires</p>
                        </div>
                    ` : `
                        <div class="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6 px-4">
                            <div class="flex items-center gap-6">
                                <div class="w-20 h-20 rounded-2xl bg-white/60 backdrop-blur-sm shadow-sm flex items-center justify-center text-${analytics.status === 'excellent' ? 'emerald' : analytics.status === 'good' ? 'blue' : 'amber'}-600">
                                    <i data-lucide="${analytics.icon}" class="w-10 h-10"></i>
                                </div>
                                <div>
                                    <div class="flex items-center gap-2 mb-1">
                                         <span class="text-xs font-black uppercase tracking-widest opacity-60">Projection Fin de Parcours</span>
                                    </div>
                                    <h3 class="text-4xl font-black mb-1 leading-none">${analytics.projectedDateStr}</h3>
                                    <p class="font-bold text-lg opacity-80">${analytics.message}</p>
                                </div>
                            </div>
                            <div class="bg-white/40 px-8 py-4 rounded-2xl backdrop-blur-sm border border-white/20 text-center min-w-[150px]">
                                <span class="block text-5xl font-black">${analytics.remainingCount}</span>
                                <span class="text-[10px] font-bold uppercase tracking-wider opacity-60">Étapes<br>restantes</span>
                            </div>
                        </div>
                    `}
                </div>
            ...
            <div class="flex flex-col gap-6 h-full">
                ...
                <!-- JAUGE -->
                <div class="flex-1 bg-orange-500 p-8 rounded-[2rem] shadow-xl text-white flex flex-col items-center justify-center relative overflow-hidden min-h-[220px]">
                    <div class="relative z-10 mb-4">
                        ...
                        <div class="absolute inset-0 flex flex-col items-center justify-center">
                            <span class="text-4xl font-black">${analytics.percent}%</span>
                        </div>
                    </div>
                    <div class="text-center relative z-10">
                        <p class="text-orange-100 font-bold uppercase tracking-widest text-sm">Progression Globale</p>
                    </div>
                </div>

                <!-- VELOCITE -->
                <div class="flex-1 bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm flex flex-col items-center justify-center text-center min-h-[180px]">
                    <div class="p-3 bg-emerald-100 text-emerald-600 rounded-xl mb-4"><i data-lucide="zap" class="w-6 h-6"></i></div>
                    <p class="text-5xl font-black text-slate-900 leading-none mb-2">
                        ${analytics.validatedCount > 0 ? (Math.round(analytics.daysSinceStart / analytics.validatedCount) + 'j') : '-'}
                    </p>
                    <p class="text-xs text-slate-400 font-bold uppercase tracking-widest">Moyenne par étape</p>
                </div>

                <!-- HISTORIQUE -->
                <div class="flex-1 bg-slate-100 p-8 rounded-[2rem] border border-slate-200 flex flex-col overflow-hidden min-h-[200px]">
                    <div class="flex items-center gap-3 mb-6 flex-shrink-0 justify-center">
                        <div class="p-2 bg-white text-slate-600 rounded-lg shadow-sm"><i data-lucide="history" class="w-4 h-4"></i></div>
                        <h3 class="font-black text-slate-900 text-sm uppercase tracking-wider">Activités Récentes</h3>
                    </div>
                    <div class="space-y-3 flex-1 overflow-y-auto pr-1 custom-scrollbar">
                        ${recentHistory.length > 0 ? recentHistory.map(h => `
                            <div class="flex items-center gap-4 p-4 bg-white rounded-2xl shadow-sm border border-slate-200/50">
                                <div class="w-8 h-8 rounded-full bg-emerald-100 text-emerald-600 flex items-center justify-center font-bold text-xs flex-shrink-0"><i data-lucide="check" class="w-4 h-4"></i></div>
                        ...
                        `).join('') : '<p class="text-center text-xs text-slate-400 py-4 italic">Rien à signaler pour le moment.</p>'}
                    </div>
                </div>
